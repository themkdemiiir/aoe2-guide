...
if (process.argv.length > 2) {
  console.error(`aggregate-rich: unexpected args ${process.argv.slice(2).join(" ")} — the --relic live-merge was removed; run the refresh-* scripts for live data.`);
  process.exit(1);
}
...
console.log(`civ-meta: 1v1 ${(L["1v1"].totalApp / 1e6).toFixed(1)}M · team ${(L.team.totalApp / 1e6).toFixed(1)}M · ${r1} civs 1v1 / ${rt} team · ${out.patches.length} patches → ${OUT}`);
