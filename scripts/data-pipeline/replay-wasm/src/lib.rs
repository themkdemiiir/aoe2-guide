...
//! replay_rs, so this needs no network and no filesystem.
//!
...
pub fn analyze_replay(data: &[u8], you: &str) -> Result<String, JsError> {
    ...
    let sel = if you.trim().is_empty() {
        replay_rs::analyze::YouSel::Auto
    } else {
        let id: i64 = you
            .trim()
            .parse()
            .map_err(|_| JsError::new("profile id must be a number"))?;
        replay_rs::analyze::YouSel::ProfileId(id)
    };
    let report = replay_rs::analyze::analyze(&game, &sel)
