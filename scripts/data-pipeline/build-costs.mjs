...
const out = {
  _source: "aoe2techtree data/data.json",
  unit: pick("unit", src.Unit),
  building: pick("building", src.Building),
  tech: pick("tech", src.Tech),
};
// Fail loud on an ABSENT Cost (data gap) — a present empty/zero Cost is legitimate and passes.
if (missing.length) {
  throw new Error(
    `build-costs: ${missing.length} entr${missing.length > 1 ? "ies" : "y"} with ABSENT Cost (data gap, not free): ${missing.join(", ")}`,
  );
}
writeFileSync("scripts/data-pipeline/replay-rs/data/costs.json", JSON.stringify(out));
console.log(
  `costs.json: ${Object.keys(out.unit).length} units, ${Object.keys(out.building).length} buildings, ${Object.keys(out.tech).length} techs`,
);
