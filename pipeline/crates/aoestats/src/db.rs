...
/// non-numeric `game_id`). The `CASE` fragment itself is GENERATED by
/// [`pipeline_core::opening::aoestats_opening_kind_case`] (see [`insert_players_sql`]) — the same
/// helper migration `m20260706_000018_add_opening_kind`'s backfill `CASE` is generated from too,
/// so the two can no longer hand-drift apart (this was a hand-duplicated copy before the
/// follow-up to final-review finding #1).
fn insert_players_sql() -> String {
    format!(
        r#"
...
SELECT
    ...
    s.winner,
    s.opening,
    ({opening_kind_case})::opening_kind,
...
"#,
        opening_kind_case = pipeline_core::opening::aoestats_opening_kind_case("s.opening")
    )
}
