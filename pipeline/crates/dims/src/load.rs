...
//! ~150 rows, civs/civs_relic ~50, patch_index ~20), so one round trip per table is plenty, and
//! `ON CONFLICT DO UPDATE` means re-running this after a refdata change (a new DLC civ, a map
//! rename) refreshes every row's slug/label/family in place. All four loads share ONE
//! transaction: a failure partway through (e.g. a malformed refdata date) rolls back everything,
//! never leaving the dims half-loaded.
...
/// [`pipeline_core::civs::load_relic_civs`], and `patch_index` from
/// [`pipeline_core::patch::load`]. Re-running with unchanged refdata leaves every table's row
