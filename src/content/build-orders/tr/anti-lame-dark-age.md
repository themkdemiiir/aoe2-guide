---
slug: anti-lame-dark-age
name: "Anti-Lame Fast Up Dark Age"
difficulty: advanced
targetAge: feudal
durationMin: 8
civsRecommended: []
steps:
  - { villagers: 6,  assign: "6 → TC altındaki koyunlara", icons: ["food", "town-center"] }
  - { villagers: 7,  assign: "+1 → ilk yaban domuzunu çek", icons: ["food"] }
  - { villagers: 9,  assign: "+2 → oduna, kütük kampı kur", icons: ["wood", "lumber-camp"] }
  - { villagers: 10, assign: "+1 → 2. yaban domuzunu çek, TC altında ye", icons: ["food", "town-center"] }
  - { villagers: 11, assign: "+1 → ikinci yaban domuzunu bitir", icons: ["food"] }
  - { villagers: 12, assign: "+1 → ev kur, sonra yabani meyvelerin üzerine değirmen", icons: ["house", "mill", "food"] }
  - { villagers: 12, assign: "Keşifçi: en az 2 geyiği TC'ye çek", note: "Zorunlu — bu, build'in özüdür", icons: ["food"] }
  - { villagers: 17, time: "8:25", assign: "+5 → TC altında yemeğe; Loom araştır (ya da +1'i oduna gönder), sonra Feudal'a tıkla", note: "Yaklaşık 8:25'te geçiş (2 odun / 15 yemek)", icons: ["food", "loom"] }
  - { villagers: 17, assign: "Feudal geldi — köylüleri avdan çekip 8 köylü tek bir kütük kampında olana dek oduna al", icons: ["wood", "lumber-camp"] }
  - { villagers: 17, assign: "2 → TC yakınındaki dağınık ağaçlara", icons: ["wood"] }
  - { villagers: 17, assign: "2 → Kışla ve bir ev kur", icons: ["barracks", "house"] }
  - { villagers: 17, assign: "Okçu açılışı: 4 köylüyü altına gönder; ya da Scout/Mızraklı/Skirmisher yap ve köylüleri yemek ve odunda tut", icons: ["archer", "scout-cavalry"] }
source:
  author: "Hera"
  url: "https://www.patreon.com/heraaoe2"
---

# Anti-Lame Fast Up Dark Age

Kendi yemeğinin her kırıntısını güvene al — iki yaban domuzunu da Şehir Merkezi altında, geyikleri de zorunlu olarak çekerek — ki hiçbir rakip seni lame'leyip güvenilir, uygarlıktan bağımsız hızlı Feudal'ından edemesin.

## Strateji ve Devamı (Sırada Ne Var)

- Bütün mesele ekonomini engellenemez kılmak: iki yaban domuzunu da TC altında yemek ve geyikleri çekmek, yemeğini düşman keşifçilerinin ve köylülerinin çalamayacağı ya da iteleyemeyeceği bir yere biriktirir; böylece seni kim lame'lemeye kalkarsa kalksın geçiş zamanın tutarlı kalır.
- Burada hiçbir şey bir uygarlık bonusuna bağlı olmadığı için, bu açılış her uygarlıkta güvenilirdir — Feudal'a yaklaşık 8:25'te temiz bir 19-pop (Loom'suz) ya da 18-pop (Loom'lu) varış bekle.
- Build bilinçli olarak yalın çalışır: Feudal'a kısa, yemek ağırlıklı bir itiş olduğundan, Dark Age'de fazladan şeylere aşırı yatırım yapma. Avı topla, çık ve oyunu devamının belirlemesine izin ver.
- Feudal'da av köylülerini tek bir kütük kampına kaydır, 8 köylü odun kesene dek; sonra 2'sini dağınık ağaçlara, 2'sini de Kışla ve eve ayır — bu, her birim seçimi için sana odun ve bina temelini verir.
- Devamını keşifte gördüğüne göre belirle: bir Okçu Menzili açılışı 4 köylüyü doğrudan altına ister, oysa Scout, Mızraklı ya da Skirmisher planı köylülerini bunun yerine yemek ve odunda tutar.
- Saldırgan keşfe ve lame'lemeye karşı bir sigorta açılışı olarak en güçlüsüdür; oyun sessizleşirse zamanında çıkmış olursun ve standart bir Feudal ya da Castle planına sorunsuzca geçebilirsin.
