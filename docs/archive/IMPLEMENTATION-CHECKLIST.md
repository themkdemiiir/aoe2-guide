...
- [ ] **P0·S [SUPERSEDED]** Unit content has been migrated to bilingual YAML (`src/content/units/<slug>.yaml`); the old `src/content/units/en/*.md` files no longer exist. The structured `unit-stats` render pulls from `src/data/unit-stats.json` (knight = 60f/75g, range 0) — verify the stat block still renders correctly without the md body table.
- [ ] **P1·M [OPEN]** Backfill unique-unit stats: **40 of 61** unique units have no `unit-stats` entry (arambai, boyars, camel-archer, condottiero, coustillier, konnik, legionary, leitis, obuch, organ-gun, ratha, serjeant, …) → all show "stats pending" via `UniqueUnitBlock.astro:78`. Source from `aoe2techtree` `data/data.json` through `build-units.mjs` (regenerates `src/data/unit-stats.json` only — data-only; unit content is the bilingual YAML).
